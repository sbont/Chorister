...
const functions = {
    ...
    getAll: <Persistable extends ApiEntityIn>(path: string) =>
        instance.get<Array<Persistable>>(path, functions.getGetConfig(path)),

    getAllRelated: <Persistable extends ApiEntityIn>(uri: string, association: string) =>
        instance.get<Array<Persistable>>(`${uri}/${association}`, functions.getGetConfig(association)),

    getOne: <Persistable extends ApiEntityIn>(uri: string) => instance.get<Persistable>(uri),

    create: <Persistable extends ApiEntityIn>(path: string, entity: Persistable) =>
        instance.post<Persistable>(path, entity),

    update: <Persistable extends ApiEntityIn>(uri: string, entity: Persistable) =>
        instance.patch<Persistable>(uri, entity),

    delete: <Persistable extends ApiEntityIn>(entity: Persistable) =>
        entity._links?.self.href ? instance.delete(entity._links?.self.href) : Promise.resolve(),
};
